...
**低置信边界条目（12 篇）处置**：逐条复核后**全部采纳路由判定**——它们均落在已裁决的边界规则上（R/N 看可用性声明、E/R 看实验发现是否为主体、A/G 看教学是否为主体），规则适用正确，不确定性来自体裁本身混合而非规则误用。清单：cerebras×3（autoresearch-loop-cheating→R、multi-agent-workflows→G、never-loop-without-verifiers→A）、databricks×2（memalign→N、scaling-small-llms-mps→R）、eleuther×3（deep-ignorance→R、aletheia-retrospective→R、dynamical-models→A）、google-research×1（connectomics-milestone→N）、microsoft-research×1（echoverse→R）、openai×1（monitor-coding-agents→R）、kezhongke×1（eval-harness-landscape→A）。若未来统计结论对某篇敏感，优先复查此清单。

**低置信条目裁决收口（2026-09-15）**：v2 扩容新增 16 条低置信经主编复核**全部维持现判**（理由：与 v1 复裁逻辑一致，低置信来自体裁混合性而非规则误用；逐条理由见当日复核记录）。全部 28 条（v1 12 条 + v2 16 条）已从各库 genre-labels.json 的 `_meta.low_confidence` 移入 `_meta.adjudicated`（decision=maintain + 裁决注记），per-article JSON 的 unified.confidence 同步升 high。体裁归属均未变，无需重聚合。
