...
> 数据来源：output/microsoft-research/_aggregate.json（10 篇全量统计；`two_level` 键为两层机构值/体裁 delta，2026-09-14 起为 §3 定量预算来源）+ 同目录 10 篇单篇 JSON + annotations/ 3 篇语步标注（orchard 24 段 / echoverse 59 段 / verifying-rust 44 段，共 127 段）+ output/_moves-aggregate.md 跨库聚合。
> 证据分级（每节开头标注）：**[统计层]** = output/ 下 JSON 全库统计（§3 定量预算自 2026-09-14 起以 `two_level` 键为准），可直接信；**[标注-3篇]** = rubric 01 语步标注，实际覆盖 3/10 篇；**[推断]** = 仅档案散文或语料通读支持，未经脚本核验。
...
> **2026-09-14 两层改造**：预算来源从"全库 median 一层"改为两层——机构层（跨体裁不变量）+ 体裁 delta（格中位 − 机构层）。数据源：output/microsoft-research/_aggregate.json 的 `two_level` 键（aggregate_two_level.py，10 篇当前口径，genre.unified.code 分格：N 公告类 n=5 / R 研究类 n=4 出值；E 工程类 n=1——verifying-rust——n<3 缺席）。旧全库口径表留痕为 §3.3，不再作硬约束来源。
> 口径标注："两层机构值" = N/R 两格中位的中位（两格时即两格中位的均值；不是全库 median）——体裁构成平衡化，防主导格（N 格 5 篇占全库 50%）绑架。

### 3.1 机构层预算（跨体裁不变量，所有体裁成稿共用）

**[统计层]** 全部抄自 two_level.institution / dispersion / signature_candidates。"判定"列：不变量·硬约束=跨体裁离散度低的机构签名候选（起点阈值：格内全距 < 跨机构全距一半；本库 18 项指标全部命中起点阈值——仅两格出值时格内全距天然偏小，候选判定是起点不是终局，故按 range 相对机构值幅度逐行校准，理由写进行内）；按 delta=range 相对机构值过大或格间差异显著，预算按 §3.2 分体裁执行；只记录=伪零或体裁专用。

| 指标 | 两层机构值 | 全库朴素 median（对照） | 跨体裁 range | 判定 | 写作指令 |
|---|---|---|---|---|---|
| 篇幅（词） | 1,701 | 1,634 | 250.5 | 不变量（±体裁 delta） | 目标 ≈1,700；N 格 ≈1,576 / R 格 ≈1,827（±125，见 §3.2）；超长篇例外机制不变——深度管线文（echoverse 4,880）须大纲声明理由 |
| 阅读时长（分钟） | 5.75 | 5.5 | 1.5 | 不变量 | 目标 5–6.5 分钟读完，随篇幅格浮动 |
| 平均句长（词） | 23.1 | 22.55 | 1.05 | **不变量·硬约束** | 长句为常态，勿切碎；格间 ±0.5 词无写作意义 |
| P90 句长（词） | 36.5 | 36 | 3.0 | **不变量·硬约束** | 允许一成句子达 35+ 词；R 格放宽至 38（delta） |
| 平均段长（词） | 43.95 | 44.4 | 4.3 | **不变量·硬约束** | 2-3 句短段为主；格间 ±2.15 词不具写作意义 |
| 数字密度 /千词 | 22.02 | 26.87 | 12.95 | 体裁决定（按 delta） | range 达机构值 59%：N 格 ≈28.5（发布类数字锚密）/ R 格 ≈15.5 |
| 精确数字 /千词 | 7.22 | 7.47 | 0.92 | 不变量（±体裁 delta） | 结果句必带精确数字；两格 ≈6.8–7.7 |
| 年份锚点 /千词 | 0.95 | 0.91 | 0.07 | **不变量·硬约束** | 两格一致（≈0.9–1.0），全文 1-2 处年份即可 |
| 限定语（hedge）/千词 | 8.36 | 7.5 | 2.75 | 起点阈值命中，校准为按 delta | range 达机构值 33%：N 格 ≈7.0 / R 格 ≈9.7；克制比按格执行（见绝对化行） |
| 绝对化断言 /千词 | 1.72 | 1.81 | 2.18 | 体裁决定（按 delta） | N 格 0.63 vs R 格 2.81（4.5 倍差）——克制比 hedge:absolutist 随之分裂：N ≈11:1 / R ≈3.5:1 |
| 全称量词 /千词 | 5.15 | 4.53 | 1.3 | 不变量（±体裁 delta） | all/every/any 适度；两格 ≈4.5–5.8 |
| 第一人称（次/篇） | 8.75 | 9 | 0.5 | **不变量·硬约束（指纹）** | **指纹指标：七库最低**（旧全库口径 vs 库群 ~26，p=0.001，q=0.002 过 FDR；2026-09-14 两层口径复核注记见 STYLE-PROFILE 指纹行——体裁构成校正后未过 FDR 但同格方向保持，仍为本库新口径最显著信号）。主语让位给系统名；两格差仅 0.5（N 9 / R 8.5）——指纹非体裁构成伪影 |
| 客观自指（this paper 等，次/篇） | 0.25 | 0.5 | 0.5 | 不变量 | 基本不用；R 格 0.5、N 格清零 |
| 问句（个/篇） | 0.5 | 0 | 1.0 | 不变量（±体裁 delta） | 默认不设问、不反问；R 类成稿允许 ≤1 个（delta） |
| 感叹号 | **0** | 0 | 0 | **不变量·红线** | 禁用，跨格零离散 |
| MATTR(150) | 0.70 | 0.70 | 0.01 | **不变量·硬约束** | 词汇多样但术语复现集中；跨格最稳定指标之一 |
| TTR | 0.38 | 0.38 | 0.01 | 不变量（仅库内自比） | 长度敏感内参，不作目标 |
| 链接 / 文内引用 / 文献条目 | 0 | 0 | 0 | 只记录 | ⚠️ 抓取归一化伪零，跨格亦无离散；资源落点强度以标注层为准（文末冲突记录 #2 不变） |

> **机构层 vs 全库朴素 median 差异简注**（差异 = 体裁构成，非同库漂移；朴素 median 含 E 工程类 1 篇（n<3 未入格），机构层不含）：
> ① 问句 0→0.5（朴素 median=0，相对差不可比；绝对差 +0.5 个/篇）：N 格 5 篇（占全库 50%）中位 0、R 格 4 篇中位 1.0——朴素 median 被零值多数压到 0，格等权后暴露 R 格水平。绝对量小，"默认不设问"纪律不变。
> ② 客观自指 0.5→0.25（−50%）：N 格清零、R 格 0.5；朴素 median 落 R 侧值，格等权取中回落。绝对差 0.25 次/篇，不改变"基本不用"纪律。
> ③ 数字密度 26.87→22.02（−18%）：N 格 5 篇（28.49）占全库 50%、占入格篇 56%，朴素 median 被其上拉；R 格 15.54 经等权恢复权重——两层差最大的实质指标。
> ④ 全称量词 4.53→5.15（+13.7%）：R 格 5.8 高于 N 格 4.5；朴素 median 中 N 格占半将其压低，等权后向 R 格回升。
> ⑤ 限定语 7.5→8.36（+11.5%）：同④构成——R 格 9.73 vs N 格 6.98。

> **人称硬约束**（P1 横评实证升级，2026-09-08；[6, 15] 区间来自旧全库 P25–P75，两层口径下机构层 8.75、按 §3.2 分体裁执行——N 格 9、R 格 8.5，两格差仅 0.5，指纹纪律不变）：次数类有长度依赖，按实际篇幅折算（±20% 篇幅差异对应 ±20% 次数调整）；写作时**按节预分配**（开场/主体/收尾各多少处 we/our，大纲阶段声明）；完稿必须过 `profiler first_person_count` 自检。**题材豁免制**：题材本身要求团队构建叙事（build-log/复盘）时可上浮，但须在大纲声明"人称豁免：目标 ~X 次，上浮原因 Y"——未声明的超界=失格（实证：P1 同题材盲测 anthropic 成稿 57 vs 基线 30，题材 we 本能压过库预算；openai 58/53 贴上沿）。本库指纹指标——超界直接损害库身份，豁免从严（仅方案引入句式可保）。

### 3.2 分体裁 delta 预算（机构层 ± delta）

**[统计层]** delta = 体裁格中位 − 两层机构值（two_level.cells − institution）。空格纪律：E 工程类（n=1，verifying-rust）n<3 缺席——**写作时暂用机构层值 + 人工判断，标 [小样本]**，不补 0、不套旧印象。R 格含 echoverse（genre.unified 置信度 low）：不推翻、计入格值，引用 R 格 delta 时保留人工复核。unified 六分与 §1 三类候选子体裁非同一套划分，对应关系待裁决（§6 缺口 2），不互套。

| 指标 | 机构层 | N 公告类（n=5） | R 研究类（n=4） |
|---|---|---|---|
| 篇幅（词） | 1,701 | 1,576（−125） | 1,827（+125） |
| 阅读时长（分钟） | 5.75 | 5.0（−0.75） | 6.5（+0.75） |
| 平均句长（词） | 23.1 | 23.6（+0.5） | 22.6（−0.5） |
| P90 句长（词） | 36.5 | 35（−1.5） | 38（+1.5） |
| 平均段长（词） | 43.95 | 46.1（+2.15） | 41.8（−2.15） |
| 数字密度 /千词 | 22.02 | 28.49（+6.47） | 15.54（−6.48） |
| 精确数字 /千词 | 7.22 | 7.68（+0.46） | 6.76（−0.46） |
| 年份锚点 /千词 | 0.95 | 0.91（−0.04） | 0.98（+0.03） |
| 限定语（hedge）/千词 | 8.36 | 6.98（−1.38） | 9.73（+1.37） |
| 绝对化断言 /千词 | 1.72 | 0.63（−1.09） | 2.81（+1.09） |
| 全称量词 /千词 | 5.15 | 4.50（−0.65） | 5.80（+0.65） |
| 第一人称（次/篇） | 8.75 | 9（+0.25） | 8.5（−0.25） |
| 客观自指（次/篇） | 0.25 | 0（−0.25） | 0.5（+0.25） |
| 问句（个/篇） | 0.5 | 0（−0.5） | 1（+0.5） |
| 感叹号 | 0 | 0（±0） | 0（±0） |
| MATTR / TTR | 0.70 / 0.38 | 0.71 / 0.38（+0.01 / ±0） | 0.70 / 0.37（±0 / −0.01） |

（链接 / 文内引用 / 文献条目两格均 0，系抓取归一化伪零，只记录不入 delta 表。）

**分体裁写作含义**（方向与 JSON 数值一致）：

- **N 公告类（n=5）——数字锚更密 + 绝对化压得更低**：数字密度 +6.47（≈28.5/千词，约为 R 格 1.8 倍）、精确数字 +0.46、段长 +2.15、P90 −1.5（≤35）；反向：限定语 −1.38（≈7.0/千词）、绝对化 −1.09（≈0.63/千词，克制比 ≈11:1）、问句 −0.5（清零）、客观自指 −0.25（清零）、篇幅 −125。含义：公告/发布类成稿数字预算上调至 ≈28/千词（密集数字锚是 N 格签名），同时绝对化压至 ≈0.6/千词——克制比远宽于机构层 4.9:1；不设问、不用 this paper 式自指。
- **R 研究类（n=4）——hedge 上浮 + 断言配额放宽 + 篇幅更长**：限定语 +1.37（≈9.7/千词）、绝对化 +1.09（≈2.8/千词，克制比 ≈3.5:1——仍克制，但断言配额约为 N 格 4.5 倍）、篇幅 +125（≈1,827）、阅读时长 +0.75、P90 +1.5（≤38）、问句 +0.5（≈1）、客观自指 +0.25（≈0.5）；反向：数字密度 −6.48（≈15.5/千词）。含义：研究类成稿 hedge 上调、数字密度下调至 ≈15.5/千词；允许 ≤1 个问句与偶发 this paper 式自指；篇幅可至 ≈1,800 词不触顶。
- **E 工程类（n=1，verifying-rust）**：n<3 缺席，写作时暂用机构层 + 人工判断，标 [小样本]。

### 3.3 旧全库口径预算表（留痕对照，2026-09-14 起不作硬约束来源）

**[统计层]**（全部抄自 _aggregate.json；唯二例外：词数分位数 aggregate 未收，由 10 篇单篇 JSON 现算补齐，来源已注明）

> **留痕说明**：本表为 2026-09-08 旧全库一层口径（10 篇 median [P25–P75]）原样保留；数值与当前 two_level.corpus_naive_median 一致（10 篇口径未漂移）——变化的是口径层级（全库一层 → 两层），非数值本身。硬约束一律以 §3.1/§3.2 两层值为准。人称硬约束纪律单源化至 §3.1，本表不再夹注。
...
2. **子体裁未正式路由**：规则层 genre 全部 `blog_or_exploration`，rubric 00 LLM 复核未跑，三类划分仅为候选——**部分缓解（2026-09-14）**：genre.unified 已回填（llm-routing-v1），§3 两层分格按 unified code 出值（N 公告类 5 / R 研究类 4 / E 工程类 1 缺席；echoverse 低置信不推翻）；但 §1 三类候选子体裁（发布文类型学）与 unified 六分非同一套划分，对应关系仍待裁决。
3. **链接/引用指标伪零**：`links_per_1k`/`citation_marks`/`reference_entries` 全 0 系抓取归一化所致，资源落点强度目前只能靠标注层（APPLY 8 处）旁证，定量表该行不可用于校验成稿。
4. **_aggregate.json 未收词数分位数**：篇幅预算由单篇 JSON 现算补齐（median 1,634 / P25 1,160 / P75 2,230），尚未入聚合管线——**部分缓解（2026-09-14）**：two_level 键已含 word_count 机构层（1,701.25）与格中位（N 1,576 / R 1,826.5），§3.1/§3.2 篇幅预算不再依赖现算补丁；P25–P75 分位数仍未入聚合管线（§3.3 留痕表 ¹ 注保留）。
