...
| **指纹**（discriminate.py，vs 余六库 pooled，对照组=当前 7 库+重采语料） | **平均段长最低**（p<0.001，q=0.001，过 FDR）——短段快节奏；对照组敏感史：第一人称低→P90句长低→全称量词高→平均段长低（见校准日志）。**2026-09-14 两层口径复核：体裁构成校正后未过 FDR，原信号含构成成分**（N 格内段长仍最低 33.2 vs 同格 pooled 46.2，p=0.021，q=0.351，探索性；新口径最显著信号=数字密度高 @N格 42.45 vs 28.2，p=0.009，q=0.311） | — | — | — | — | — | — |
