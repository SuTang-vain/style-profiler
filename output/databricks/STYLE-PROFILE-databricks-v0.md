...
> 生成日期：2026-09-07 ｜ 语料：20 篇（2026-09-14 体裁平衡扩容 10→20；原 10 篇：2026-01 至 2026-08，AI RESEARCH 栏目全量；扩容篇目量具变更见 §2 ‡ 注记与 §6 校准日志 2026-09-15 条）｜ 抓取：Kimi WebBridge 真实浏览器
...
| **指纹**（discriminate.py，vs 余六库 pooled，7 库限定） | — | — | **TTR 最高**（内参指标（q=0.002）；MATTR 7/7 终态定稿：0.71 次高（Google 0.73 最高）且对 OpenAI（0.68）显著高——"词汇直白"成立但非库群唯一极值；TTR 行按内参保留）。**2026-09-14 两层口径复核：体裁构成校正后未过 FDR，原信号含构成成分**（R 格内 TTR 0.44 vs 同格 pooled 0.34，p=0.014，q=0.311；新口径最显著信号=第一人称高 @N格 28.0 vs 9.0，p=0.013，q=0.311，探索性）。**2026-09-15 扩容复核（n=20，FDR 族分列口径不变）**：最显著信号换位年份锚点低 @N格（0.17 vs pooled 2.24，p=0.004，q=0.148），仍未过 FDR；旧口径对照（5b）最显著由 TTR 高换位年份锚点低（q<0.001 过 FDR，仅对照）。MATTR 新中位 0.71（Google/MSR 0.72 并列最高） | — | — | — | — |
...
> ‡ 量具变更（2026-09-14 扩容）：新 10 篇因官网订阅表单改版缺少国家下拉样板（约 500 词/篇），word_count 新旧篇目系统性不可比——本列为跨量具版本中位（方向：新篇偏轻），篇幅衍生读数按带偏口径使用。
...
- 2026-09-14：两层口径复核（discriminate.py 体裁对体裁改造）。旧指纹"TTR 最高（内参）"体裁构成校正后未过 FDR（R 格内 p=0.014，q=0.311），按纪律保留原行并加构成成分注记；新口径最显著信号为第一人称高 @N格（28.0 vs 同格 pooled 9.0，p=0.013，q=0.311，探索性，待扩容复验）。
- 2026-09-15：扩容重聚合（Phase B1）。语料 10→20 篇（2026-09-14 体裁平衡扩容，新篇 router=llm-routing-v2-expansion；旧 JSON 未重跑，genre.unified 不受影响）。**量具变更（必须随数值阅读）**：新 10 篇因官网订阅表单改版缺少国家下拉样板（约 500 词/篇），word_count 新旧篇目系统性不可比——聚合值跨量具版本，方向：新篇偏轻；§2 篇幅行已加 ‡ 注记，篇幅衍生读数（阅读时长类）按带偏口径使用，必要时按单篇 JSON 分组回看。顶层重算（profiler.aggregate()，等价性双验证通过），§2 表刷新：篇幅 2,076→2,476.5‡、句长 26.2→23.75、第一人称 21.5→30.5、限定语 5.5→5.28、断言 1.11→1.57、客观自指 0.0→0.5、TTR 0.46→0.40；数字密度 24.4→24.38、精确数字 4.0、MATTR 0.71 基本不变；其余五列同步扩容后口径（壳中客列保持 v10）。two_level：N4/R6 → E4/G3/N6/R7（新增 E/G 格）。discriminate（FDR 族分列口径不变）：最显著信号由第一人称高 @N格（q=0.311）换位年份锚点低 @N格（0.17 vs pooled 2.24，p=0.004，q=0.148），仍未过 FDR；旧口径对照最显著由 TTR 高换位年份锚点低（q<0.001，仅对照）。
