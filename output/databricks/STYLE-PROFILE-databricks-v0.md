...
| **指纹**（discriminate.py，vs 余六库 pooled，7 库限定） | — | — | **TTR 最高**（内参指标（q=0.002）；MATTR 7/7 终态定稿：0.71 次高（Google 0.73 最高）且对 OpenAI（0.68）显著高——"词汇直白"成立但非库群唯一极值；TTR 行按内参保留）。**2026-09-14 两层口径复核：体裁构成校正后未过 FDR，原信号含构成成分**（R 格内 TTR 0.44 vs 同格 pooled 0.34，p=0.014，q=0.311；新口径最显著信号=第一人称高 @N格 28.0 vs 9.0，p=0.013，q=0.311，探索性） | — | — | — | — |
