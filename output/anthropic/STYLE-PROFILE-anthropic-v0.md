...
|---|---|---|---|
| 篇幅 | 2,639.5 词 | 3,164 词 | 4,830 字 |
| 平均句长（词/字） | 22.45 词 | 22.3 词 | 34.5 字 |
| **数字密度 /千词(字)** | **8.25**（概念驱动） | 20.69 | 17.08/千字 |
| 精确数字 | 1.46 | 0.92 | 5.58（最高） |
| hedge /千词(字) | 10.52 | 9.57 | 2.69/千字 |
| 断言型绝对化 | 2.32 | 1.76 | 1.08 |
| **第一人称** | 30.0 | **53** | 6.0 |
| 参考文献条目 | 0 | 0 | **10.5**（0–51） |
| 外链 /千词 | 0 | 0 | 0（行业分析除外） |
| 感叹号 | 0（10 篇共 6 个） | 0（共 6 个） | 0 |
| TTR | 0.34（长度敏感；MATTR 0.69） | 0.33（MATTR 0.68） | 0.35（MATTR 0.74） |
| **指纹**（discriminate.py，vs 其余六库 pooled，MATTR 7/7 终态） | **数字密度最低**（8.25 vs ~25，p=0.001，q=0.002，过 FDR）——概念驱动安全叙事（回到 5 库时代的结论）。**2026-09-14 两层口径复核：体裁构成校正后未过 FDR，原信号含构成成分**（R 格内数字密度仍最低 8.8 vs 同格 pooled 24.5，p=0.075，q=0.422；E 格内不显著 p=0.513；新口径最显著信号=限定语高 @R格 14.91 vs 8.67，p=0.042，q=0.351，探索性） | — | — |

> 指纹解读：概念驱动的安全叙事——七库中每千词数字最少，论证靠情景剧与边界声明而非数据堆砌。
| 标题冒号式 | 1/10（祈使句/名词短语） | 3/10 | 5/10 |
