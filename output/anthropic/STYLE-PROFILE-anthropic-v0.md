...
> 生成日期：2026-09-07 ｜ 语料：20 篇（2026-09-14 体裁平衡扩容 10→20；原 10 篇：engineering ×6、research ×3、news/政策 ×1）｜ 抓取：浏览器直连（无 Cloudflare 拦截）
...
## 2. 量化基线（20 篇，median；2026-09-15 扩容重聚合）· 三方对照表

| 指标 | Anthropic | OpenAI | 壳中客 v10（中文，10 篇） |
|---|---|---|---|
| 篇幅 | 2,195 词 | 1,601.5 词 | 4,830 字 |
| 平均句长（词/字） | 23.8 词 | 22.4 词 | 34.5 字 |
| **数字密度 /千词(字)** | **8.84**（概念驱动） | 17.77 | 17.08/千字 |
| 精确数字 | 0.6 | 0.9 | 5.58（最高） |
| hedge /千词(字) | 10.52 | 10.16 | 2.69/千字 |
| 断言型绝对化 | 2.3 | 1.77 | 1.08 |
| **第一人称** | 30.0 | **40** | 6.0 |
| 参考文献条目 | 0 | 0 | **10.5**（0–51） |
| 外链 /千词 | 0 | 0 | 0（行业分析除外） |
| 感叹号 | 0（20 篇共 6 个） | 0（共 6 个） | 0 |
| TTR | 0.35（长度敏感；MATTR 0.69） | 0.35（MATTR 0.68） | 0.35（MATTR 0.74） |
| **指纹**（discriminate.py，vs 其余六库 pooled，MATTR 7/7 终态） | **数字密度最低**（8.25 vs ~25，p=0.001，q=0.002，过 FDR）——概念驱动安全叙事（回到 5 库时代的结论）。**2026-09-14 两层口径复核：体裁构成校正后未过 FDR，原信号含构成成分**（R 格内数字密度仍最低 8.8 vs 同格 pooled 24.5，p=0.075，q=0.422；E 格内不显著 p=0.513；新口径最显著信号=限定语高 @R格 14.91 vs 8.67，p=0.042，q=0.351，探索性）。**2026-09-15 扩容复核（n=20，FDR 族分列口径不变）**：最显著信号换位限定语高 @E格（10.01 vs pooled 5.47，p=0.015，q=0.234），仍未过 FDR；旧口径对照（5b）保持数字密度最低（q<0.001 过 FDR，仅对照） | — | — |

> 指纹解读：概念驱动的安全叙事——七库中每千词数字最少，论证靠情景剧与边界声明而非数据堆砌。
| 标题冒号式 | 1/20（祈使句/名词短语） | 4/20 | 5/10 |
...
| ❌ | 篇幅压缩（790 词级） | 最短 890 词（responsible-scaling-policy），展开充分是本体特征 |
...
3. **人称光谱**：OpenAI 53（调查者 we）> Anthropic 30（责任者 we）> 壳中客 6.0（机构客观）——第一人称含量与"研究过程即内容"的程度正相关。
...
- 2026-09-08：对照表与散文全量同步当前统计层（openai/anthropic 系 v2 重采口径）。Anthropic 列：篇幅 2,622→2,639.5（分派权威值 2,640 为其取整）、第一人称 42.5→30.0、hedge 9.6→10.52、数字密度 7.4→8.25、TTR 0.35→0.34、MATTR 注记削除并填 0.69、感叹号总数 4→6；OpenAI 列：第一人称 59→53、句长 22.2→22.3、数字密度 16.1→20.69、hedge 8.8→9.57、TTR 0.31→0.33、MATTR 填 0.68、感叹号总数 2→6；壳中客列整体切换 v10 口径（10 篇）：篇幅 3,905→4,830 字、句长 33.6→34.5、数字密度 18.1→17.08、精确 5.07→5.58、hedge 3.1→2.69、absolutist 1.05→1.08、第一人称 5→6.0、参考文献 6→10.5、TTR 0.37→0.35（MATTR 0.74）、标题冒号 9/24→5/10；指纹行 7.4 vs ~22→8.25 vs ~25（p=0.001/q=0.002 经 discriminate.py 重跑复核不变，MATTR 覆盖 7/7）；反例行"最短 1533 词"→890 词；人称光谱 59/42.5/5→53/30/6.0。
- 2026-09-14：两层口径复核（discriminate.py 体裁对体裁改造）。旧指纹"数字密度最低"体裁构成校正后未过 FDR（R 格内 p=0.075，q=0.422；E 格 p=0.513），方向在 R 格保持但显著性消失，按纪律保留原行并加构成成分注记；新口径最显著信号为限定语高 @R格（14.91 vs 同格 pooled 8.67，p=0.042，q=0.351，探索性）。

> 校准日志（v1.1，2026-09-08）：语料重采入本地 corpus/anthropic/（含 3 个 slug 修正，见 corpus/README.md），表格数值以重采实测校准；MATTR 补齐（median 0.691）。
- 2026-09-15：扩容重聚合（Phase B1）。语料 10→20 篇（2026-09-14 扩容补采自 anthropic.com/news，新篇 router=llm-routing-v2-expansion；旧 JSON 未重跑，genre.unified 不受影响）。顶层重算（profiler.aggregate()，等价性双验证通过），§2 表刷新：篇幅 2,639.5→2,195 词、句长 22.45→23.8、数字密度 8.25→8.84、精确数字 1.46→0.6、断言 2.32→2.3、TTR 0.34→0.35、感叹号"10 篇共 6 个"→"20 篇共 6 个"、标题冒号 1/10→1/20；hedge 10.52、第一人称 30.0、MATTR 0.69 不变；OpenAI 列同步扩容后口径（壳中客列保持 v10）。two_level：E3/G3/R3 → A4/E3/G3/N4/P3/R3（新增 A/N/P 格）。discriminate（FDR 族分列口径不变）：最显著信号由限定语高 @R格（q=0.351）换位限定语高 @E格（10.01 vs pooled 5.47，p=0.015，q=0.234），仍未过 FDR；旧口径对照保持数字密度最低（q<0.001，仅对照）。
