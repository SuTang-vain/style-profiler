...
1. **MATTR 0.713 出带上沿（+0.013）**——三路评审一致判轻微：题材跨双域专名密集（NRSRO/Dodd-Frank/SWE-bench）是结构性原因；实测仍在 anthropic/openai 库内文件范围内（两库 max 0.743/0.724）。暴露一个 **playbook 真空：§0.2 只写了"贴下沿"，高侧超标无处方**——继续压只能撞处方 3（术语统一）或处方 7（禁排比重述）。
2. **主题词对偶过载**（新病灶类，_p2 未记录）："the graded" 名词化 ×12，一处同句三 "grade" 词族叠加——建议 fluency pass 变体 2–3 处；已记入待办。
3. **信源分层执行最薄**：一处事实断言未挂来源（"According to reporting last year"）；EU AI Act 罚款比例一处建议信源复核。
4. **年份锚点 4.19/千词 vs A 格 delta −1.0 方向未达成**：历史对照题材的结构性证据需求；走廊未设此项，不越界，但若走廊未来补收年份项本篇将出带（judge-quantitative §5 存档）。
