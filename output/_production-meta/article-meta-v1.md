...
Nor was openai a special case. Under the pooled caliber, all seven English libraries in our corpus had produced a statistically significant fingerprint; under the cell-corrected caliber, zero of seven did.
...
This repository exists to read style with data: eight libraries — seven English, plus the kezhongke library, this site's own Chinese corpus — 164 articles, twenty-two metrics per article, each routed into one of six unified genre cells. This analysis answers three questions about what surfaced when the reading broke. First, how large is the distortion that genre composition can inject into an institutional style claim. Second, how the two-level model corrects it, and why the correction itself deserves trust. Third, what discipline a practitioner should adopt before quoting anyone's style profile, ours included.
...
The six cells in our taxonomy — analysis, engineering, research, announcement, policy, and guide — exist precisely so that this comparison can be made inside a genre before it is made across institutions. Our old caliber pooled anyway: every article a library had published was lumped together and tested against the other six libraries, metric by metric. Our corpus made the trap easy to enter, because in the ten-article era seven of the ten openai articles sat in engineering or research cells, while the eleuther library contributed eight research articles and almost nothing else.
...
## 1. Seven for seven, then zero of seven

The pooled caliber was flattering to everyone. The old fingerprint section of our discrimination report lists all seven libraries passing false-discovery correction, with q-values at or below the one-per-mille level: cerebras on paragraph length, microsoft-research on first-person voice, and so on down the table. Seven libraries, seven signatures, seven clean certificates of distinctiveness — and, we would argue now, readings of one confound.

The corrected caliber compares genre cell to genre cell: composition is treated as a lens whose curvature we can map, not a flaw to polish out. Pairwise tests run inside each cell under their own correction family, and the institutional fingerprints form a separate family of their own: library by shared cell by metric, 420 tests at the current sample size. The split matters, because correction is priced per family: a signal that would clear a single pooled batch could fail once the test family honestly mirrors the structure of the comparison.

Under this caliber the fingerprint table reads, seven times in a row: exploratory signal, did not pass correction, awaiting sample expansion. The closest approach is eleuther's low lexical diversity in the research cell, a MATTR of 0.62 against a same-cell pooled 0.70. Its p-value rounds to zero; its q-value, after the family takes its share, is 0.088. The openai library's leading signal under this caliber is no longer first-person voice at all but hedge density in the research cell, 15.1 against a same-cell pooled 7.38, and it fails correction as well, at q of 0.234.

How to word that result responsibly is a discipline of its own, and the expansion put every wording to a direct test.

The expansion supplied that test: we doubled every English library from ten to twenty articles, and the fingerprint family more than doubled with it. The conclusion did not move: zero of seven, and the nearest signal remains eleuther's, still labeled exploratory. Why the correction takes this particular shape — medians of medians — is the subject of the next section.
...
Attribution sharpened as well: in a leave-one-out exercise, where we assign each article to its nearest institutional centroid, accuracy rose from 63 percent across the pooled corpus to 75 percent inside the research cell — both figures from the ten-article stage; at the current twenty-article size the same exercise runs on the announcement cell and lands at 65 percent. Strip out the composition contamination and the libraries became easier to tell apart, not harder, which is roughly what a confound predicts.

The institution value is not a constant of nature; it is a function of the populated cell set. After the expansion, the openai precise-digit comparison flipped: pooled median 0.9, institution value 0.42, because the cell set had grown from two populated cells to five. We keep both calibers on record for exactly this reason, and the repository keeps the earlier revisions as well. A correction that never changed its answer would likely be a cosmetic; ours moves with the evidence, and each move is logged.
...
## 4. Does the corrected lens work
...
Five rules, each executable before lunch. First, stratify before you compare: behind any cross-institution style number, ask for the genre mix, and compare inside shared cells or treat the number as a description of the mix rather than of the institution. Second, ask which caliber produced the figure, and keep both calibers on record: a number without one is not a measurement; it is an anecdote with decimals.

Third, treat every significant fingerprint as exploratory until it survives the right correction family: the family has to mirror the comparison structure, because one pooled batch spread over mixed genres manufactures confidence. Fourth, audit the instrument before the data — word lists need substring checks, diversity metrics need length controls, routers need coverage tests, and each known failure deserves a standing sentinel test. Fifth, expand with a tripwire rather than a hope: decide which archived signal, at which q-value, would change your mind, and write it down before the next batch of articles arrives.
...
## So is your own profile an artifact
...
Two questions the evidence does not cover: whether stable institutional fingerprints may emerge at around forty articles per library, and whether subtype splits redraw the cells underneath the argument; the current record answers neither.

This essay was drafted under the playbook it describes; if published, it will join the corpus it analyzes, and that is as it should be. Genre composition is a lens that sits permanently in the beam of any style measurement: you do not remove it, you map its curvature and correct for the bend. We have mapped ours once, logged the correction, and left both calibers on the shelf. Do the same to yours.

## Sources

- output/_group-discrimination.txt — the discrimination report: section 4b (cell-corrected fingerprint family) and section 5b (pooled caliber kept for contrast).
- output/openai/STYLE-PROFILE-openai-v0.md — the openai archive's fingerprint row, carrying both calibers and the research-cell first-person figures.
