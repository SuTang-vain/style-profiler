...
| 数字密度 | 20.7 | 18.1（相当）|
| 精确数字 | 0.92（低——用数量级不用精确值）| 5.07（高——精确值癖）|
...
| TTR | 0.31（长度敏感；MATTR 待语料重采后重算）| 0.37 |
| **指纹**（discriminate.py，vs 其余六库 pooled，MATTR 7/7 终态） | **第一人称最高**（59，p=0.002，q=0.003，过 FDR）——侦探叙事 "we" 主体；精确数字最低（0.92）退居次显著 | — |
