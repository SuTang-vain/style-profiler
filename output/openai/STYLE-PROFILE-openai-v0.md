...
|---|---|---|
| 篇幅 | 3,164 词 ≈ 中文学 6.3K 字 | 4,148 字（v0.1 24 篇口径，cjk 中位）|
| 平均句长 | 22.3 词 | 33.6 字（≈中等偏长，两库相当）|
| 数字密度 | 20.7 | 18.7（相当）|
| 精确数字 | 0.92（低——用数量级不用精确值）| 5.09（高——精确值癖）|
| 限定语 hedge | **9.6** | 3.1（OpenAI 高 3 倍）|
| 断言型绝对化 | 1.76 | 1.05 |
| 第一人称 | **53** | 6（OpenAI 高约 9 倍）|
| 客观自指 | 1.0 | 1.5 |
| 外链/千词 | **0** | 0（median；但壳中客行业分析有内文外链）|
| 参考文献条目 | **0** | 6（壳中客有规范参考节）|
| 感叹号 | 0（median；10 篇共 6 个）| 0 |
| TTR | 0.33（长度敏感）；MATTR 0.68 | 0.37 |
| **指纹**（discriminate.py，vs 其余六库 pooled，MATTR 7/7 终态） | **第一人称最高**（53，p=0.002，q=0.003，过 FDR）——侦探叙事 "we" 主体；精确数字最低（0.92）退居次显著 | — |

> 指纹解读：侦探叙事用数量级而非精确值——数字密度（20.7）不低但精确值极少，唯一精确值留给关键证据（0.1%–15.4%）。
